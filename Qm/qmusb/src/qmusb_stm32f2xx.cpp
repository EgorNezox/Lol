...
bool QmUsb::getStatus()
{
	bool isPowerUsb = hal_gpio_get_input((hal_gpio_pin_t){hgpioPA, 9}) == 1;
	bool isCableUsb = hal_gpio_get_input((hal_gpio_pin_t){hgpioPA, 10}) == 1;
	volatile bool isObjectInit = gethUsbDeviceFS(); // hUsbDeviceFS.pClassData != NULL;
	return (isPowerUsb && isCableUsb && isObjectInit);
}
