...
static const char* saghenTitleStr      = "********* ÑÀÆÅÍÜ-Í *********\r\n\n\0";
static const char* bootloaderTitleStr  = "     -= Çàãðóç÷èê =-   \r\n\n\n\0";
static const char* usbComputerTitleStr = "    -= USB Êîìïüþòåð =-   \r\n\0";
static const char* sysBootTitleStr     = "  -= Ñèñòåìíûé Çàãðóç÷èê =-  \r\n\0";

static const char* firmwareStr    = " Ãëàâíàÿ\r\n\n\0";
static const char* usbComputerStr = " USB Êîìïüþòåð\r\n\n\0";
static const char* sysBootStr     = " Ñèñò.Çàãð.\r\n\n\0";
...
static void drawFirmwareErrorScreen(uint8_t firmwareNum)
{
    ...
    switch (firmwareNum)
    {
    	case 0: QmConsoleScreen::oprintf("\t\t\t\t\t\t USB Êîìïüþòåð\r\n\r\n\t\t\t\t\is ïîâðåæäåí/îòñóòñòâóåò "); break;
    	case 1: QmConsoleScreen::oprintf("\t\t\t\t\t\t\t\t\t\  Îñíîâíàÿ\r\n\r\n\t\t\t\t\is ïîâðåæäåíà/îòñóòñòâóåò "); break;
    }
}
...
void qmMain()
{
	...
	switch (test_result)
	{
		case hwboottestOk:
		{
			QmConsoleScreen::oprintf("\n Áàçîâûé àïïàðàòíûé òåñò ïðîéäåí\r\n");
			break;
		}
		case hwboottestErrorExtSram:
		{
			QmConsoleScreen::oprintf("   Òåñò âíåøíåé ïàìÿòè ÍÅ ÏÐÎÉÄÅÍ   ");
			QmConsoleScreen::oprintf("  ! Ñèñòåìà îñòàíîâëåíà !");
			while(1); // cannot safely continue !
			break;
		}
		case hwboottestErrorHseClock:
		{
			QmConsoleScreen::oprintf("  Òåñò òàêòèðîâàíèÿ ÍÅ ÏÐÎÉÄÅÍ   ");
	      	QmConsoleScreen::oprintf("  ! Ñèñòåìà îñòàíîâëåíà !");
			while(1); // cannot safely continue !
			break;
		}
