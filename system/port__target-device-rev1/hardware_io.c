...
//			| (6 << POSITION_VAL(FSMC_BTR1_DATAST))
			| (3 << POSITION_VAL(FSMC_BTR1_ADDSET))
			| (1 << POSITION_VAL(FSMC_BTR1_ADDHLD))
			| (3 << POSITION_VAL(FSMC_BTR1_DATAST))
...
void stm32f2_ext_pins_deinit(int platform_hw_resource) {
	...
	case platformhwAtuUart:
		...
		break;
	case platformhwAtuIopin:
		hal_gpio_deinit((hal_gpio_pin_t){hgpioPA, 8});
		break;
	case platformhwBatterySmbusI2c:
...
//на ножку  PA1 приходят импульсы с ноги 1PSS (PG7),
//мы захватываем и замеряем сигнал по фронтам
void timer2_init()
{
	hal_gpio_params_t params;
	hal_gpio_set_default_params(&params);
	...
	hal_gpio_init((hal_gpio_pin_t){hgpioPA, 1}, &params);
