...
public:
    AleCom();
    static void generate_call(int8s* data, bool line_type, int8s cycle_num, int8s resp_addr);
    static void generate_resp_call_qual(int8s* data, int8s err_num, int8s snr);
    static void generate_trans_mode(int8s* data, int8s sound_type, int8s work_mode, int8s param_mode, bool schedule, int8s call_addr);
    static void generate_msg_head(int8s* data, int16s msg_size);
    static void generate_resp_pack_qual(int8s* data, bool pack_result, int8s snr);
